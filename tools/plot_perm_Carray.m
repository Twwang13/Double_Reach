function [] = plot_perm_Carray
load('data/fit_result_fitnlm.mat',...
    'rs_adj','d_fitmodel_full','d_rs_add','d_rs_full','d_rs_multi');
...
t = 1:size(rs_adj,2);
% ADD n
R = smooth(nanmean(d_rs_add(41:end,:),1))';
E = std(d_rs_add(41:end,:),1,'omitnan')/sqrt(size(d_rs_add(41:end,:),1));
plot(R,'linewidth',2,'color',[8 118 191]/255); hold on
fill([t fliplr(t)],[R+2*E fliplr(R-2*E)],[8 118 191]/255,'LineStyle','none');alpha(.3);
% COSINE
R = smooth(nanmean(rs_adj(41:end,:),1))';
E = std(rs_adj(41:end,:),1,'omitnan')/sqrt(size(rs_adj(41:end,:),1));
plot(R,'linewidth',2,'color',[66 66 66]/255); hold on
fill([t fliplr(t)],[R+2*E fliplr(R-2*E)],[66 66 66]/255,'LineStyle','none');alpha(.3);
% FULL
R = smooth(nanmean(d_rs_full(41:end,:),1))';
E = std(d_rs_full(41:end,:),1,'omitnan')/sqrt(size(d_rs_full(41:end,:),1));
plot(R,'linewidth',2,'color',[217 83 25]/255); hold on
fill([t fliplr(t)],[R+2*E fliplr(R-2*E)],[217 83 25]/255,'LineStyle','none');alpha(.3);
% MULTI
R = smooth(nanmean(d_rs_multi(41:end,:),1))';
E = std(d_rs_multi(41:end,:),1,'omitnan')/sqrt(size(d_rs_multi(41:end,:),1));
...
p=[];r=[];
for i=1:length(R)
multi = d_rs_multi(41:end,i);
add = d_rs_add(41:end,i); %%%%%%%%%%%%%%%%%%%在这里把那三个点删去再做一次显著性分析
[p(i),~,stats] = signrank(add,multi,'method','approximate');
r(i) = stats.zval/sqrt(length(add));
end
% Msig = double(p<0.0005 & r<=0); Msig(Msig<1)=nan;
% plot(Msig,'color',[130 54 146]/255,'linewidth',2);
% Asig = double(p<0.0005 & r>=0); Asig(Asig<1)=nan;
...
a = squeeze(mean(abs(d_coff_full(41:end,:,[1 3 4 5])),1));
t = 1:size(rs_adj,2);
coff = abs(d_coff_full(41:end,:,[1 3 4 5]));E=[];
for i=1:4
...
set(gca,'xtick',3.5:10:83.5);ylabel('Abs. Offset');ylim([0.2,0.6]);ax.YTick=[0.2,0.6];
% ax.XTickLabel = {'-0.8','','-0.4','','0','','0.4','','0.8'};
ax.XTick=[];
s(6)=subplot(4,3,6);
for i=1:3
...
box off
ax = gca;xlim([3.5,83.5]);ylim([0,0.2]);ax.YTick=[0,0.2];
...
multi = d_rs_multi(41:end,t_bin);
add = d_rs_add(41:end,t_bin);
...
annotation('textbox',[.42 .83 .2 .1],'String',['Wilcoxon signed rank test p=' num2str(p(t_bin))],'EdgeColor','none','FontSize',12);


%% plot permutation
% P test
load('data/Perm_result_fitnlm_1st.mat')
perm_rs(:,:,:,1) = d_rs_full;
perm_coff(:,:,:,:,1) = d_coff_full;
load('data/Perm_result_fitnlm_mul.mat')
perm_rs(:,:,:,2) = d_rs_full;
perm_coff(:,:,:,:,2) = d_coff_full;
load('data/Perm_result_fitnlm_2nd.mat')
perm_rs(:,:,:,3) = d_rs_full;
